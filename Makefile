...
wait-to-be-healthy:
	@docker inspect --format='{{json .State.Health.Status}}' kong-gateway | jq -r
...
kong-db: kong-init
	...
	@sleep 5 # TODO: db have a healthcheck?
...
kong-oss: kong-destroy kong-db
	...
	@./scripts/wait-for-healthy-container.sh kong-gateway
	@make sync
